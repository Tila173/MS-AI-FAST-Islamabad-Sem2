...
SUBSET_TAG = "20"   # change to "10" if your generated files are actually 10%
...
OUT_DIR = f"/mnt/optimusmesh/checkingfiles/meld_project/outputs/checkpoints/text_emotion_{SUBSET_TAG}"
...
def save_all_features(model, loader, split_name):
    ...
    save_features(
        f"{FEATURE_DIR}/text_emotion_{SUBSET_TAG}_{split_name}.pt",
        ids_all,
        feats_all,
        labels_all
    )
...
def save_metrics_json(train_loss, train_acc, dev_loss, dev_acc, test_loss, test_acc):
    out_path = f"{RESULTS_DIR}/text_emotion_{SUBSET_TAG}_metrics.json"
    metrics = {
...
def run():
    ...
    best_dev_acc = 0.0
    best_path = f"{OUT_DIR}/best_text_emotion.pt"
