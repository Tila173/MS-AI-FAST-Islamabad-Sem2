...
OUT_DIR = f"/mnt/optimusmesh/checkingfiles/face_outputs_emotion_{SUBSET_TAG}"
RESULTS_DIR = "/mnt/optimusmesh/checkingfiles/results"
FEATURE_DIR = "/mnt/optimusmesh/checkingfiles/features"
CACHE_DIR = f"/mnt/optimusmesh/checkingfiles/face_cache_emotion_{SUBSET_TAG}"
...
class FaceDataset(Dataset):
    def __init__(self, csv_path, video_root):
        ...
        for _, row in self.df.iterrows():
            ...
            if os.path.exists(video_path) and label in EMOTION2ID:
...
class FaceModel(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Linear(512, len(EMOTION2ID))
...
def save_all_features(model, loader, split_name):
    ...
    save_features(
        f"{FEATURE_DIR}/face_emotion_{SUBSET_TAG}_{split_name}.pt",
        ids_all,
        feats_all,
        labels_all
    )
...
def save_metrics_json(train_loss, train_acc, dev_loss, dev_acc, test_loss, test_acc):
    out_path = f"{RESULTS_DIR}/face_emotion_{SUBSET_TAG}_metrics.json"
    metrics = {
        "task": "emotion",
...
def run():
    ...
    best_dev_acc = 0.0
    best_path = f"{OUT_DIR}/best_face_emotion.pt"
