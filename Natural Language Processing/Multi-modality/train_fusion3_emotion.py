...
OUT_DIR = "/mnt/optimusmesh/checkingfiles/fusion3_outputs_emotion"
...
SUBSET_TAG = "20"
AUDIO_TAG = "15"
MODALITIES = ["text", "audio", "video"]  # change this trio if needed
...
def save_metrics_json(train_loss, train_acc, dev_loss, dev_acc, test_loss, test_acc):
    out_path = f"{RESULTS_DIR}/fusion3_emotion_metrics.json"
    metrics = {
        "task": "fusion3_emotion",
...
def run():
    ...
    if len(MODALITIES) != 3:
        raise ValueError("Set exactly 3 modalities in MODALITIES.")

    splits = ["train", "dev", "test"]
    raw_data = {
        modality: {
            split: load_feature_file(feature_path("emotion", modality, split))
            for split in splits
        }
    ...
    best_dev_acc = 0.0
    best_path = f"{OUT_DIR}/best_fusion3_emotion.pt"
