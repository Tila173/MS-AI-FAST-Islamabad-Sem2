...
OUT_DIR = "/mnt/optimusmesh/checkingfiles/fusion_outputs_emotion"
...
def save_metrics_json(train_loss, train_acc, dev_loss, dev_acc, test_loss, test_acc):
    out_path = f"{RESULTS_DIR}/fusion_emotion_metrics.json"
    metrics = {
        "task": "fusion_emotion",
        "text_tag": SUBSET_TAG,
        "audio_tag": AUDIO_TAG,
...
def run():
    ...
    text_train = load_feature_file(f"{TEXT_FEATURE_DIR}/text_emotion_{SUBSET_TAG}_train.pt")
    text_dev = load_feature_file(f"{TEXT_FEATURE_DIR}/text_emotion_{SUBSET_TAG}_dev.pt")
    text_test = load_feature_file(f"{TEXT_FEATURE_DIR}/text_emotion_{SUBSET_TAG}_test.pt")

    audio_train = load_feature_file(f"{FEATURE_DIR}/audio_emotion_{AUDIO_TAG}_train.pt")
    audio_dev = load_feature_file(f"{FEATURE_DIR}/audio_emotion_{AUDIO_TAG}_dev.pt")
    audio_test = load_feature_file(f"{FEATURE_DIR}/audio_emotion_{AUDIO_TAG}_test.pt")

    video_train = load_feature_file(f"{FEATURE_DIR}/video_emotion_{SUBSET_TAG}_train.pt")
    video_dev = load_feature_file(f"{FEATURE_DIR}/video_emotion_{SUBSET_TAG}_dev.pt")
    video_test = load_feature_file(f"{FEATURE_DIR}/video_emotion_{SUBSET_TAG}_test.pt")

    face_train = load_feature_file(f"{FEATURE_DIR}/face_emotion_{SUBSET_TAG}_train.pt")
    face_dev = load_feature_file(f"{FEATURE_DIR}/face_emotion_{SUBSET_TAG}_dev.pt")
    face_test = load_feature_file(f"{FEATURE_DIR}/face_emotion_{SUBSET_TAG}_test.pt")
    ...
    model = FusionMLP(input_dim=input_dim, num_classes=len(EMOTION2ID)).to(DEVICE)
    optimizer = torch.optim.AdamW(model.parameters(), lr=LR)
    criterion = nn.CrossEntropyLoss()

    best_dev_acc = 0.0
    best_path = f"{OUT_DIR}/best_fusion_emotion.pt"
