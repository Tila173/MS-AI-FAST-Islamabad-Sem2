...
TRAIN_CSV = f"{BALANCED_DIR}/train_{SUBSET_TAG}_emotion.csv"
DEV_CSV   = f"{BALANCED_DIR}/dev_{SUBSET_TAG}_emotion.csv"
TEST_CSV  = f"{BALANCED_DIR}/test_{SUBSET_TAG}_emotion.csv"
...
OUT_DIR = "/mnt/optimusmesh/checkingfiles/audio_outputs_emotion_15"
...
class AudioDataset(Dataset):
    def __init__(self, csv_path, audio_root):
        ...
        for _, row in self.df.iterrows():
            ...
            label = str(row["Emotion"]).strip().lower()

            if os.path.exists(audio_path) and label in EMOTION2ID:
    ...
    def __getitem__(self, idx):
        ...
        label = EMOTION2ID[str(row["Emotion"]).strip().lower()]
...
class AudioModel(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Linear(self.encoder.config.hidden_size, len(EMOTION2ID))
...
def save_all_features(model, loader, split_name):
    ...
    save_features(
        f"{FEATURE_DIR}/audio_emotion_15_{split_name}.pt",
        ids_all,
        feats_all,
        labels_all
    )
...
def save_metrics_json(train_loss, train_acc, dev_loss, dev_acc, test_loss, test_acc):
    out_path = f"{RESULTS_DIR}/audio_emotion_15_metrics.json"
    metrics = {
...
def run():
    ...
    dev_loader   = DataLoader(dev_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=2, collate_fn=collate_fn)
    ...
    best_dev_acc = 0.0
    best_path = f"{OUT_DIR}/best_audio_emotion.pt"
